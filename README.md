...
7. Paste the API URL and token into the connector tab, then click `Save / reconnect`
   (`Enable connector` is checked by default).
...
- `Disconnect` stops polling and cancels local activity, but keeps the saved token until you
  replace it or remove the local connector config yourself.
